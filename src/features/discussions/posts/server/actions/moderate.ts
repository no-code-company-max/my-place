...
async function togglePostHidden(
  ...
  // G.3: gate atómico permission-groups. `hasPermission` con FALLBACK
  // (G.1.5–G.7) reconoce role===ADMIN y owner; drop fallback en G.7.
  const allowed = await hasPermission(actor.actorId, actor.placeId, 'discussions:hide-post')
  if (!allowed) {
    throw new AuthorizationError('No tenés permiso para ocultar/revelar posts.', {
      postId: post.id,
    })
  }
