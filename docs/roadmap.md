...
- **R.2 — App shell común (`_shell`)** — R.2.0 → R.2.3 cerrados; R.2.5.0 → R.2.5.4(A) cerrados; R.2.5.4(B) manual QA mobile real pending (depende de deploy a Vercel).
  ...
  - **R.2.6 — FAB cross-zona con menú contextual** — sub-fase R.2.6.0 cerrada; R.2.6.1+ pendientes.
    - **R.2.6.0 — Spec del FAB** ✅ (2026-04-26). `docs/features/shell/spec.md` § 17 (11 sub-secciones cubriendo objetivos, arquitectura 2 capas, specs visuales, posicionamiento desktop-aware, visibilidad, menú items, a11y, z-index, conflictos, componentes, sub-fases) + ADR `docs/decisions/2026-04-26-zone-fab.md` (4 decisiones formalizadas + alternativas). Decisiones consultadas al user: reemplaza CTAs (no coexisten), solo zonas root, ícono `Sparkles` (no `+` ni hamburger por escalabilidad + evitar choque con swipe). Marca `docs/features/discussions/spec.md` § 21.8 follow-up FAB ✅. Implementa la promesa explícita del spec § 21.8.
    - **R.2.6.1+ (pendientes)**: `<FAB>` primitive en `shared/ui/` + `<ZoneFab>` orquestador en `shell/ui/` + tests; mount en `(gated)/layout.tsx` + remover CTAs de threads/events headers + update skeletons + update E2E specs (`post-crud`, `events-create-rsvp`, extender `hours-gate`, nuevo `zone-fab.spec.ts`); cleanup + manual QA + roadmap ✅. Sub-milestones detallados en spec § 17.11. Total estimado 3-4 sesiones.
