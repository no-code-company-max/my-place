...
- **R.2 — App shell común (`_shell`)** — R.2.0 → R.2.3 cerrados; R.2.5.0 cerrado, R.2.5.1+ pendientes.
  ...
  - **R.2.5 — Swipe horizontal entre zonas** — sub-fase R.2.5.0 cerrada.
    - **R.2.5.0 — Spec del swiper** ✅ (2026-04-26). `docs/features/shell/spec.md` § 16 (11 sub-secciones cubriendo objetivos, modelo route-based, ZoneSwiper, eliminación skeleton, freshness con Next 15 staleTimes, scroll preservation, accesibilidad, robustez producción, componentes, sub-fases, excepciones) + ADR `docs/decisions/2026-04-26-zone-swiper.md` (9 decisiones formalizadas + alternativas). Decisiones consultadas al user: `framer-motion@^11` (vs hand-rolled / @use-gesture), bounce elástico (vs hard-stop / cycle), freshness vía Next route cache + refresh condicional (recomendada por mí con cost analysis: 5K WS messages/place no se justifican; Realtime per-zona DIFERIDO). Gap crítico descubierto en audit: Next 15 cambió route cache default a 0s para dinámicas — requiere `experimental.staleTimes.dynamic: 30` explícito. Otros gaps integrados: per-zona scroll preservation, prefers-reduced-motion, error boundary, gesture testing strategy split (unit en vitest + Playwright para touch), bundle delta monitoreado.
    - **R.2.5.1+ (pendientes)**: install framer-motion + componentes (`<ZoneSwiper>`, `<SwiperViewport>`, `<TopProgressBar>`); configurar staleTimes + mount en `(gated)/layout.tsx` + remover `loading.tsx` de zonas root; prefetch on dot focus/hover + `lastVisitedAt` cache + scroll preservation; E2E Playwright + manual QA mobile real; cleanup + docs. Sub-milestones detallados en spec § 16.10. Total estimado 5-6 sesiones.
